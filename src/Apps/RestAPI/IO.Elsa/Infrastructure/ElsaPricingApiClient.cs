...
public class ElsaPricingApiClient : IElsaPricingApiClient
{
        ...
        ILogger<ElsaPricingApiClient> logger)
    {
        ...
        this._logger = logger;
        this._pricingApiUrl = configuration["ELSA_PRICING_API"] ?? 
        ...
        this._httpClient.DefaultRequestHeaders.Add("User-Agent", "IO.Elsa/1.0");
    }
    ...
    public async Task<ElsaPricingApiResponse> GetPriceAsync(Dictionary<string, object> requestBody)
    {
        try
        {
            this._logger.LogInformation("Calling Elsa pricing API at {Url}", this._pricingApiUrl);
            this._logger.LogDebug("Request body: {RequestBody}", JsonSerializer.Serialize(requestBody));

            var jsonContent = JsonSerializer.Serialize(requestBody);
            var content = new StringContent(jsonContent, Encoding.UTF8, "application/json");

            var response = await this._httpClient.PostAsync(this._pricingApiUrl, content);
            
            if (!response.IsSuccessStatusCode)
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                this._logger.LogError("Elsa pricing API returned status {StatusCode}: {Error}", 
            ...
            this._logger.LogDebug("Elsa pricing API response: {Response}", responseContent);
            ...
            this._logger.LogInformation("Successfully retrieved pricing data from Elsa API");
            return apiResponse;
        }
        catch (Exception ex)
        {
            this._logger.LogError(ex, "Error calling Elsa pricing API");
            throw;
        }
    }
}

...
var group = configuration.GetValue<string>(EnvironmentVariables.ApplicationGroup) ?? "job-processing";

// Configure structured logging and OpenTelemetry
builder.Services.AddStructuredLogging(configuration, "IO.Lea");
builder.Services.AddOpenTelemetryInstrumentation(configuration, "IO.Lea");

// Add environment configuration management
builder.Services.AddEnvironmentConfiguration(configuration, "IO.Lea");
...
builder.AddMongoDb()
       .AddJobListingsRepository();

// Add Google Jobs API client
builder.Services.AddSingleton<IGoogleJobsApiClient, GoogleJobsApiClient>();

// Add job service
builder.Services.AddSingleton<IJobBusinessService, JobBusinessService>();
...
{
    c.SwaggerDoc("v1", new() { Title = "IO Lea API", Version = "v1" });
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "IO Lea API v1");

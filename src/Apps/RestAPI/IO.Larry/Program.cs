...
var group = configuration.GetValue<string>(EnvironmentVariables.ApplicationGroup) ?? "vendor-management";

// Configure structured logging and OpenTelemetry
builder.Services.AddStructuredLogging(configuration, "IO.Larry");
builder.Services.AddOpenTelemetryInstrumentation(configuration, "IO.Larry");

// Add environment configuration management
builder.Services.AddEnvironmentConfiguration(configuration, "IO.Larry");
...
builder.AddMongoDb()
       .AddVendorDataRepository();

// Add vendor service
builder.Services.AddSingleton<IVendorBusinessService, VendorBusinessService>();

// Add repositories
builder.Services.AddSingleton<VendorRepository>();
builder.Services.AddSingleton<VendorPerformanceRepository>();
builder.Services.AddSingleton<VendorComplianceRepository>();
...
{
    c.SwaggerDoc("v1", new() { Title = "IO Larry API", Version = "v1" });
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "IO Larry API v1");
